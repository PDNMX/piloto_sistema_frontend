...
<head>
    <title>Code coverage report for src/cli-validator/utils/processConfiguration.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../index.html">All files</a> / <a href="index.html">src/cli-validator/utils</a> processConfiguration.js</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">92.63% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>176/190</span>
            </div>
...
<a name='L412'></a><a href='#L412'>412</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">10x</span>
...
&nbsp;
  ...
  <span class="missing-if-branch" title="else path not taken" >E</span>if (message.length) {
...
&nbsp;
          ...
          const message =
            newRule === ''
              ? <span class="branch-0 cbranch-no" title="branch not covered" >`The rule '${oldRule}' has been deprecated. It will not be checked.`</span>
...
&nbsp;
  if (defaultMode) {
    ...
    try {
      ...
      const description =
<span class="cstat-no" title="statement not covered" >        'There is a problem with the .validaterc file. See below for details.';</span>
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (configObject.invalid) {
<span class="cstat-no" title="statement not covered" >      return Promise.reject(2);</span>
    }
  }
&nbsp;
...
&nbsp;
  ...
  <span class="missing-if-branch" title="else path not taken" >E</span>if (ignoreFile === null) return [];
&nbsp;
  const pathToFile = <span class="cstat-no" title="statement not covered" >`${path.dirname(ignoreFile)}/`;</span>
&nbsp;
  let filesToIgnore;
<span class="cstat-no" title="statement not covered" >  try {</span>
    const fileAsString = <span class="cstat-no" title="statement not covered" >await readFile(ignoreFile, 'utf8');</span>
&nbsp;
    ...
    const globsToIgnore = <span class="cstat-no" title="statement not covered" >fileAsString</span>
      .split('\n')
      .filter(<span class="fstat-no" title="function not covered" >li</span>ne =&gt; <span class="cstat-no" title="statement not covered" >line.trim().length !== 0)</span>
      .map(<span class="fstat-no" title="function not covered" >gl</span>ob =&gt; <span class="cstat-no" title="statement not covered" >pathToFile + glob)</span>;
&nbsp;
<span class="cstat-no" title="statement not covered" >    filesToIgnore = await globby(globsToIgnore, {</span>
      expandDirectories: true,
      dot: true
    });
  } catch (err) {
<span class="cstat-no" title="statement not covered" >    filesToIgnore = [];</span>
  }
&nbsp;
<span class="cstat-no" title="statement not covered" >  return filesToIgnore;</span>
};
&nbsp;
...
const validateConfigOption = function(userOption, defaultOption) {
  ...
  Object.keys(configOptions).forEach(option =&gt; {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (configOptions[option].includes(defaultOption)) {
      optionType = option;
    }
  });
  // verify the given option is valid
  const validOptions = configOptions[optionType];
  <span class="missing-if-branch" title="else path not taken" >E</span>if (!validOptions.includes(userOption)) {
    result.valid = false;
...
&nbsp;
  ...
  try {
    for (const file of ruleSetFilesToFind) {
      <span class="missing-if-branch" title="else path not taken" >E</span>if (!ruleSetFile) {
...
<span class="cstat-no" title="statement not covered" >    ruleSetFile = defaultRuleset;</span>
  }
&nbsp;
  <span class="missing-if-branch" title="else path not taken" >E</span>if (!ruleSetFile) {
